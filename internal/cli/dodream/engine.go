...
import (
	...
	"github.com/mandacode-labs/dodream/internal/engine"
)
...
func runEngineServer(cmd *cobra.Command, args []string) error {
	...
	engServer, err := engine.NewServer(cfg)
	if err != nil {
	...
	grpcSrv := engine.NewGRPCServer(engServer.Processor)
	grpcServer, err := engine.StartGRPCServer(grpcAddr, grpcSrv)
	if err != nil {
